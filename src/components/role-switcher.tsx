        ...
        {open && (
          <div className="max-h-[calc(100dvh-11rem)] w-[19rem] overflow-y-auto overflow-x-hidden rounded-2xl border border-line bg-white shadow-[0_28px_60px_-18px_rgba(49,48,43,0.4)]">
            ...
            {/* How much of the profile summary is readable without
                ...
                what shipped; the rest answer the complaint that a summary
                you have to unfold is not a summary. */}
            <div className="border-t border-line px-4 py-2.5">
              <div className="text-[12.5px] font-semibold text-ink">
                Profile summary
              </div>
              <div className="text-[11.5px] text-caption">
                Staff boards — how much shows at a glance
              </div>
            </div>
            <div>
              {PROFILE_VARIANT_META.map((m, i) => {
                const active = profileVariant() === m.id;
                return (
                  <form
                    key={m.id}
                    action={setProfileVariantAction.bind(null, m.id)}
                  >
                    <button
                      className={`flex w-full items-center gap-2.5 px-4 py-2 text-left transition-colors ${
                        active ? "bg-cream/70" : "hover:bg-muted"
