...
function seedDemo(db) {
  ...
  function fillForm(appId, learner, overrides = {}) {
    const values = {
      ...
      stream_12: "Science",
      status_12: "Completed",
      completion_12: "2016-05",
      completion_year_12: "2016",
      required_score_12: "60",
      has_marksheet_12: "Yes",
      marksheet_12: "class12-marksheet.pdf",
      school_name: "Delhi Public School",
      score_12: "84",
      bachelor_status: "Completed",
      bachelor_degree: "B.E. Computer Science",
      bachelor_degree_name: "B.Tech",
      bachelor_completion: "2019-06",
      ...
      bachelor_medium: "English",
      bachelor_moi: "English",
      bachelor_naac: "A+",
      bachelor_anabin: "H+",
      required_score_bachelor: "65",
      pg_status: "No",
      work_exp_months: "48",
      cv_file: "resume.pdf",
      career_gap_months: "0",
   ...
   */
  /** The answers already written for this application, for clause blanks. */
  function responsesFor(appId) {
    const rows = db
  ...
  function addClauseDocs(appId, learnerName, signature = null) {
    ...
    for (const id of String(clauses).split("|").filter(Boolean)) {
      ...
      insertDoc.run(
        ...
        t.type,
        t.title,
        `I, ${learnerName}, ${fillPlaceholders(t.content, responsesFor(appId)).replace(/^I /, "")}`,
        1,
        signedAt,
        signature,
        t.id
  ...
  /**
   * The documents every application carries, whatever the answers. Pulled
   * from the template table rather than restated here — the clause wording
   * is the legal team's and must have exactly one home (see db.ts).
   */
  function addDefaultDocs(appId, learnerName, signature = null) {
    const signedAt = signature ? "2026-08-03 09:10:00" : null;
    const always = db
      .prepare("SELECT * FROM document_templates WHERE always_required = 1")
      .all();
